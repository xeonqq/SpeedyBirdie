...
void TransitionTo(State state) {
  ...
  Serial << micros() << _F(" Transition To:") << static_cast<int>(g_state)
         << endl;
}
void TransitionToWarmUp() { TransitionTo(State::WarmUp); }
...
void onApplyDevConfig(HttpRequest &request, HttpResponse &response) {
  if (request.method == HTTP_POST) {
    AppSettings.get<LeftMotorOffset>() =
        request.getPostParameter("left_motor_offset").toInt();
    AppSettings.get<RightMotorOffset>() =
        request.getPostParameter("right_motor_offset").toInt();
    ...
    AppSettings.get<GripperServoEndPosition>() =
        request.getPostParameter(GripperServoEndPosition::name).toFloat();
    ...
    AppSettings.get<LifterServoEndPosition>() =
        request.getPostParameter(LifterServoEndPosition::name).toFloat();
    ...
    motors.SetPwmOffsets({AppSettings.get<LeftMotorOffset>(),
                          AppSettings.get<RightMotorOffset>()});
    gripper_servo.InitByStartAndEnd(
        AppSettings.get<GripperServoStartPosition>(),
        AppSettings.get<GripperServoEndPosition>());
...
void onApplyConfig(HttpRequest &request, HttpResponse &response) {
  if (request.method == HTTP_POST) {
    AppSettings.get<ShootingPower>() =
        request.getPostParameter("power").toInt();
    AppSettings.get<ShootingIntervalSec>() =
        std::max(request.getPostParameter("interval").toFloat(),
                 GetShootingIntervalLowerBound());
    AppSettings.save();
    ...
    motors.Init(shooting_interval_sec, AppSettings.get<ShootingPower>() * 0.5,
                AppSettings.get<ShootingPower>(), false);
...
void loadConfig() {
  ...
  AppSettings.get<ShootingIntervalSec>() =
      constrain(AppSettings.get<ShootingIntervalSec>().value,
                GetShootingIntervalLowerBound(), 10);

  float shooting_interval_sec = AppSettings.get<ShootingIntervalSec>();

  gripper_servo.Init(shooting_interval_sec,
                     AppSettings.get<GripperServoStartPosition>().value,
                     AppSettings.get<GripperServoEndPosition>().value);

  ball_release_to_push_delay = AppSettings.get<BallReleaseToPushTimeDelay>();
  lifter_servo.Init(shooting_interval_sec,
                    AppSettings.get<LifterServoStartPosition>().value,
                    AppSettings.get<LifterServoEndPosition>().value);

  uint16_t shooting_power =
      constrain(AppSettings.get<ShootingPower>().value, 0, 200);
...
void setupStartStopButton() {
  ...
  attachInterrupt(StartStopButtonPin,
                  InterruptDelegate(onStartStopButtonPressed), CHANGE);
...
void init() {
  ...
  WifiAccessPoint.config(_F("SpeedyBirdie"), "chinasprung", AUTH_WPA_WPA2_PSK);
  ...
  main_loop_timer
      .initializeMs(static_cast<int>(main_loop_interval * 1000), main_loop)
      .start();
